...
def evaluate_classifier(train_pos, train_neg, val_pos, val_neg, test_pos, test_neg, source_embeds, target_embeds):
    ...
    train_auc = get_roc_score_t(train_pos, train_neg, source_embeds, target_embeds)
    ...
    train_results['SIGMOID'].extend([train_auc, train_auc])

    test_pred_true = defaultdict(lambda: [])
    val_pred_true = defaultdict(lambda: [])
    train_pred_true = defaultdict(lambda: [])
    ...
    for operator in operatorTypes:
        ...
        train_ap_score = average_precision_score(train_labels, train_predict)
        ...
        # train_mrr_score = get_MRR(train_labels, train_predict, adj, do_softmax=False)
        # val_mrr_score = get_MRR(val_labels, val_predict, adj, do_softmax=False)
        # test_mrr_score = get_MRR(test_labels, test_predict, adj, do_softmax=False)

        train_results[operator].extend([train_roc_score, train_ap_score])
        val_results[operator].extend([val_roc_score, val_ap_score])
        test_results[operator].extend([test_roc_score, test_ap_score])

        train_pred_true[operator].extend(zip(train_predict, train_labels))
        val_pred_true[operator].extend(zip(val_predict, val_labels))
        test_pred_true[operator].extend(zip(test_predict, test_labels))

    return val_results, test_results, val_pred_true, test_pred_true, train_results, train_pred_true
...
def get_roc_score_t(edges_pos, edges_neg, source_emb, target_emb):
    ...
    def sigmoid(x):
        if x >= 0:  # Optimize the sigmoid function to avoid extreme data overflow.
